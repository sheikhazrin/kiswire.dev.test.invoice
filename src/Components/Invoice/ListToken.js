...
const InvoiceTokenList = (props) => {
  // ***** setting variable
  const baseURL = 'https://61cc1f01198df60017aebeda.mockapi.io/invoice';
  const tokenURL = 'eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJ1c2VybmFtZSI6InVzZXIxIiwiZmlyc3RuYW1lIjoiVXNlciAxICIsImxhc3RuYW1lIjoiVXNlciAxIiwiZW1haWwiOiJ1c2VyMUBraXN3aXJlLmNvbS5teSIsIndvcmtncm91cGlkIjowLCJyb2xlaWQiOjAsInJvbGVsZXZlbCI6MCwicGhvdG91cmwiOiJub25lIiwic2lnbmF0dXJldXJsIjoibm9uZSIsImV4cCI6MTY0MTM3NTQ2NX0.LO1iJ89cYsxaBFlJH5TTrL1J5-vxOu8CaWcJkrSW3l0';
  ...
  const columns = [
    ...
    {
      name: 'Total',
      selector: (row) => number(row.itotal).toPrecision(2),
      sortable: true,
    },
    {
      name: 'Due',
  ...
  useEffect(() => {
    async function getData() {
      await axios.get(baseURL, { headers: {"Authorization" : `Bearer ${tokenURL}`} })
      .then((response) => {
              // console.log(response.data);
              setDatas(response.data);
              setPending(false);
...
export default InvoiceTokenList;